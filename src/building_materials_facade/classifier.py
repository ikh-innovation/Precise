"""Inference: DINOv3 Facade-8 material classifier with patch-voting.

`FacadeMaterials` is a drop-in Module 3 backend — same
`classify(region_mask=...) -> MaterialClassificationResult` and
`classify_instances(detections)` surface as `BuildingMaterials` (SigLIP2) and
`MincMaterials` — so `client.py` swaps it in by config. It samples wall patches
strictly inside Module 1's building region (via the shared region sampler, so
the red-region-only contract is enforced here too), classifies each with the
fine-tuned DINOv3 model, and averages the per-class probabilities.
"""
...
class FacadeMaterials:
    """Material classifier backed by the fine-tuned DINOv3 Facade-8 model."""
    ...
    def __init__(
        ...
        ckpt_path = Path(cfg.facade_checkpoint)
        if not ckpt_path.exists():
            raise FileNotFoundError(
                f"Facade material checkpoint not found: {ckpt_path}. Train it with "
                f"`python src/building_materials_facade/train.py`."
            )
        ckpt = torch.load(ckpt_path, map_location="cpu")
        self.class_names: list[str] = ckpt["class_names"]
        self.arch: str = ckpt.get("arch", cfg.facade_arch)
        model = build_classifier(
            len(self.class_names), arch=self.arch, pretrained=False,
            unfreeze_blocks=ckpt.get("unfreeze_blocks", 0),
            hidden=ckpt.get("hidden", 512), dropout=ckpt.get("dropout", 0.3),
        )
        ...
        self.transform = build_transform(dc["mean"], dc["std"], self.input_size, train=False)
    ...
    def _no_region_result(self, width: int, height: int) -> MaterialClassificationResult:
        ...
        return MaterialClassificationResult(
            ...
            metadata=Metadata(candidate_materials=list(self.class_names), model=f"facade-{self.arch}"),
        )
    ...
    def classify(
        ...
        patches, source = sample_region_patches(
            rgb, region_mask,
            patch_size=self.cfg.facade_patch_size,
            max_patches=self.cfg.facade_max_patches,
            min_wall_ratio=self.cfg.facade_min_wall_ratio,
        )
        if not patches:
